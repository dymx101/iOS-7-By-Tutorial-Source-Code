...
- (void)updateLabels {
    ...
    _gainLabel.text = [NSString stringWithFormat:@"%.2f", [self valueForSlider:_gainSlider]];
    _minDelayTimeLabel.text = [NSString stringWithFormat:@"%.0f", [self valueForSlider:_minDelayTimeSlider]];
    _maxDelayTimeLabel.text = [NSString stringWithFormat:@"%.0f", [self valueForSlider:_maxDelayTimeSlider]];
}
...
- (void)createAUGraph {
    ...
    effectUnitDescription.componentSubType = kAudioUnitSubType_Delay;
