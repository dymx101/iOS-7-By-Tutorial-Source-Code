    ...
    if (nil == delegate.myCard) {
        ...
        if ([[delegate.session connectedPeers] count] == 0) {
            ...
            if (kProgrammaticDiscovery) {
                ...
                browserViewController.maximumNumberOfPeers = 2; // Challenge 3
...
{
    // Check if the nearby peer has sent the right code word
    AppDelegate *delegate = (AppDelegate *) [[UIApplication sharedApplication] delegate];
    return [delegate isMatchingDiscoveryInfo:info];
}
